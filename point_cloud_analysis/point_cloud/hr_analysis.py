# point_cloud_analysis/point_cloud/hr_analysis.py
...
def analyze_hr(point_cloud, scale):
    ...
    plt.title(f'Radius vs Theta of Plant')
    plt.show()
    ...
    cursor, = ax.plot(0, 0, 'ro', markersize=1)
    ...
    print(f"Real Canopy Volume: {real_volume}")
    ...
    analysis_results = {
        'plant_height': plant_height * scale if height is not None else plant_height,
        'max_radius': max_r * scale,
        'canopy_volume_real': real_volume,
        'ratio': height / max_r if height is not None else None
    }
