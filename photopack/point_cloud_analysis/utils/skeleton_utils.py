...
def make_plot(plot_points, colors=None, point_size=1.0):
    """
    Visualize 3D points using Open3D.

    Parameters:
    - plot_points (numpy.ndarray): A Nx3 array of 3D points.
    - colors (numpy.ndarray or bool, optional): 
        - If a Nx3 array, it specifies the RGB color for each point, with values in [0, 1].
        - If False or None, all points will be colored red by default.
    - point_size (float, optional): Size of the points in the visualization window.
    """
    # Validate plot_points
    if not isinstance(plot_points, np.ndarray):
        raise TypeError("plot_points must be a NumPy array.")
    if plot_points.ndim != 2 or plot_points.shape[1] != 3:
        raise ValueError("plot_points must be a Nx3 array.")

    # Handle colors
    if colors is False or colors is None:
        # Default to red color
        colors = np.zeros_like(plot_points)
        colors[:, 0] = 1  # Red channel
    else:
        if not isinstance(colors, np.ndarray):
            raise TypeError("colors must be a NumPy array.")
        if colors.shape != plot_points.shape:
            raise ValueError("colors must have the same shape as plot_points (Nx3).")
    ...
    # Optional: Improve visualization settings
...
def plot_boxes(myDict):
    ...
    make_plot(np.array(cg), labels/255)
